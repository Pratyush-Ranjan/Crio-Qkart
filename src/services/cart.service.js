...
const addProductToCart = async (user, productId, quantity) => {
  ...
  let newcart = {"email": user.email, "cartItems": [{"product": product, "quantity": quantity}]};
  const data = await Cart.create(newcart);
  if(!data)
  {
    throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, "500 Internal Server Error! Cart creation failed");
  }
  return data;
};
...
const checkout = async (user) => {
  ...
  {
    totalamount+= cart.cartItems[i].product.cost * cart.cartItems[i].quantity;
  }
