...
const getUser = catchAsync(async (req, res) => {
  ...
  if(!data)
    ...
    throw new ApiError(httpStatus.FORBIDDEN, "authenticated user tries to access a different user’s data");
  else
  {
    if(req.query.q==="address")
    {
      const val = await userService.getUserAddressById(id);
      res.send({"address": val.address});
    }
    logger.info(data);
    res.send(data);
  }
